...
package com.mcredit.service.checktools;
...
public class CheckIdentifierService extends BasedService {
	...
	public Response getResult(@QueryParam("productGroup") String productGroup, @QueryParam("citizenId") String citizenId, 
			@QueryParam("loanAmount") Long loanAmount, @QueryParam("appNumber") String appNumber, @QueryParam("saleCode") String saleCode) throws Exception {
		try( CheckToolsManager manager = new CheckToolsManager() ) {
			return ok(manager.checkCitizenId(productGroup, citizenId, loanAmount, appNumber, StringUtils.nullToEmpty(saleCode)));
		}
	}
}
