...
public class VNPostPartner extends PartnerBase{
	...
	public HashMap<String, Audit> readDetailFile(String day, List<Audit> duplicateRecords) throws Exception {
		...
		readConfig(CacheManager.Parameters().findParamValueAsString(ParametersName.AUDIT_VNPOST_CONFIG).toString());
		...
		for (DynamicFile df: res.keySet()) {
		    ...
		    while(entries.hasMoreElements()){
		        ...
		        HashMap<String, Audit> lstAudit = CommonUtils.getTxtFiles(zip.getInputStream(entry), entry.getName(), AuditEnum.TIME_CONTROL_24h.value(), duplicateRecords);
