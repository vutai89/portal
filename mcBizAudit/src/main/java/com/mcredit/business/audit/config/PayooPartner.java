...
public class PayooPartner extends PartnerBase{
	...
	public HashMap<String, Audit> readDetailFile(String day, List<Audit> duplicateRecords) throws Exception {
		...
		readConfig(CacheManager.Parameters().findParamValueAsString(ParametersName.AUDIT_PAYOO_CONFIG).toString());
		HashMap<DynamicFile , Object> res = getFile(day);		

		for (DynamicFile key : res.keySet()) {
			result.putAll(readExcelFile(key, res.get(key), 3, ThirdParty.PAYOO.value(), AuditEnum.THU.value(), duplicateRecords));
