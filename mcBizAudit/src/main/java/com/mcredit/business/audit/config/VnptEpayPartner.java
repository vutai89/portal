...
public class VnptEpayPartner extends PartnerBase {
	...
	public HashMap<String, Audit> readDetailFile(String day, List<Audit> duplicateRecords) throws Exception {
		...
		readConfig(CacheManager.Parameters().findParamValueAsString(ParametersName.AUDIT_VNPTEPAY_CONFIG).toString());
		HashMap<DynamicFile, Object> res = getFile(day);

		for (DynamicFile key : res.keySet()) {
			result.putAll(readExcelFile(key, res.get(key), 1, ThirdParty.VNPTEPAY.value(),AuditEnum.THU.value(), duplicateRecords));
