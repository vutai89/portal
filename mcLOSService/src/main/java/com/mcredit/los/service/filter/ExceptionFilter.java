package com.mcredit.los.service.filter;
...
//				else if (ExtractorContainerException)
				...
				else if (tempEx instanceof InternalException) {
					status = ((InternalException) tempEx).getHttpStatusCode();
					
					returnCode = ((InternalException) tempEx).getCode();
					message = ex.getMessage();
				}
				else {
					status = 500;
	...
	private String replaceMessage(String message) {
		...
		return message.replace(
				...
				.replace("com.mcredit.sharedbiz.validation.InternalException: ", "");
	}
